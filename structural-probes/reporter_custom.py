...
class WordPairReporter(Reporter):
  ...
  def __init__(self, args):
    ...
    self.reporting_method_dict = {
        ...
        'by_sentence': self.report_by_sentence
        }
  ...
  def report_by_sentence(self, prediction_batches, dataset, split_name):
    import pandas as pd
    """
    Custom function 
    Writes predicted and gold distances to disk for each sentence.
    """
    rows = []
    output_df = pd.DataFrame(columns=['sentence', 'poses', 'length',
                                      'label',  'prediction', 'gold_edges', 'pred_edges',
                                      'spearmanr', 'uuas'])

    for prediction_batch, (data_batch, label_batch, length_batch, observation_batch) in tqdm(zip(
        prediction_batches, dataset), desc='[sentence-wise report]'):
      for prediction, label, length, (observation, _) in zip(
          prediction_batch, label_batch,
          length_batch, observation_batch):
        ...
        # for spearmanr
        prediction = prediction[:length,:length] # matrix
        label = label[:length,:length].cpu()
        mean_spearmanr = np.mean([spearmanr(pred, gold).correlation for pred, gold in zip(prediction, label)])
        ...
        rows.append({'sentence': words,
                     ...
                     'spearmanr': mean_spearmanr,
                     'uuas': uuas})
        
    output_df = pd.DataFrame(rows)
    output_df.to_pickle(os.path.join(self.reporting_root, split_name + '_by_sentence.pkl'))
